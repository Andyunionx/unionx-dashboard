...
class DataIngestionEngine:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.desktop_path = Path.home() / "Desktop" / "Junior Revenue"

        # Configurar credenciales de Google
        creds_file = self.current_dir / "credentials.json"
        if creds_file.exists():
            self.config['service_account_json'] = str(creds_file)
            # También configurar variable de entorno para google.auth.default()
            os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = str(creds_file)

        # Buscar Excel en carpeta actual primero, luego en Desktop
        excel_name = "Análisis Contribución 2026 V02.02.xlsx"
        if (self.current_dir / excel_name).exists():
            self.contribucion_file = self.current_dir / excel_name
        elif (self.desktop_path / excel_name).exists():
            self.contribucion_file = self.desktop_path / excel_name
        else:
            self.contribucion_file = self.current_dir / excel_name
    ...
    def run_trigger_dia10_mes(self, andres_email: str, andres_password: str) -> bool:
        ...
        print(f"\n[{self.timestamp}] Ejecutando skill 'distribucion-comisiones-canal'...")
        print(f"  (Skill ejecutará automáticamente con EERR: {eerr_path})")
        print(f"  Nota: Claude ejecutará la skill en el próximo paso")
