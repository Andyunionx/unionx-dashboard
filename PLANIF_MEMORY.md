...
- [ ] Refrescar `planif_forecast_manual.parquet` (PPTO) — data del 27-may, quedó vieja

> Nota auditoría 7-jul-26: **Turso ya no existe** (migración a Parquet+DuckDB
> completada en jun); todo corre con `PARQUET_ONLY=1`. El branch
> `feat/fc-planif-onboarding` ya está mergeado a main (la vista Cobertura vive
> en `views/planning/triada_cobertura.py`).
