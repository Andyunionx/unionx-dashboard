...
def _ventas_dia(fecha: date) -> pd.DataFrame:
    """Ventas del día desde SQLite (Turso live + historico, ~5 min).
    Solo bodega propia — excluye fulfillment externo."""
    try:
        db_path = get_local_db_path()
        sql = """
            SELECT pedido,
                   CAST(hora_venta_num AS INTEGER) AS hour,
                   cantidad,
                   bodega,
                   canal
            FROM ventas
        ...
        st.warning(f"Error cargando datos: {type(e).__name__}: {str(e)[:100]}")
        return pd.DataFrame()


@st.cache_data(ttl=300, show_spinner=False)
def _ventas_cyber_historico() -> pd.DataFrame:
    """Todos los días Cyber desde SQLite."""
    try:
        db_path = get_local_db_path()
        sql = """
            SELECT fecha_venta, pedido,
                   CAST(hora_venta_num AS INTEGER) AS hour,
                   cantidad, bodega, canal
            FROM ventas
        ...
        try:
            df = pd.read_sql_query(sql, conn,
                                   params=(CYBER_START.isoformat(), CYBER_END.isoformat()))
        finally:
            conn.close()
        df["fecha_venta"] = pd.to_datetime(df["fecha_venta"]).dt.date
        return df
    except Exception as e:
        st.warning(f"Error histórico: {type(e).__name__}: {str(e)[:100]}")
        return pd.DataFrame()
...
def _tab_hoy(meta: dict, fecha: date):
    ...
    with col_btn:
        if st.button("🔄 Actualizar", key="cyber_refresh_hoy", use_container_width=True):
            _ventas_dia.clear()
    with col_info:
        st.caption("Ventas en vivo desde SQLite + Turso (~5 min lag). Solo bodega propia, sin fulfillment externo.")

    df = _ventas_dia(fecha)

    if df.empty:
        st.warning("Sin ventas de bodega propia registradas para hoy.")
...
def _tab_acumulado(meta: dict):
    ...
    if df.empty:
        st.info("Sin datos históricos del Cyber aún.")
        return
    ...
    c1.metric("Pedidos acumulados", f"{ped_total:,}".replace(",", "."))
    ...
    df_show = by_dia[["dia_nombre", "fecha_venta", "pedidos",
                       "uds", "meta_uds", "avance_pct"]].copy()
...
def render():
    ...
    if hoy < CYBER_START:
        dias = (CYBER_START - hoy).days
        st.info(f"⏳ El Cyber arranca en **{dias} días** ({CYBER_START.strftime('%d/%m/%Y')}).")
    elif hoy > CYBER_END:
    ...
    tab_hoy, tab_acum, tab_metas = st.tabs([
        "⚡ Hoy en vivo", "📊 Acumulado Cyber", "🎯 Metas por canal",
    ])
