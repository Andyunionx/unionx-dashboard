"""
🔐 Login SSO Odoo para la app Contabilidad — sesión persistente HMAC 8h.
Patrón idéntico a _ops_auth y _fin_auth, con:
  - CONT_ALLOWED_EMAILS (Andrés + contadora externa)
  - Cookie "unionx_cont_session" separada
"""
import hashlib
import hmac
import os
...
COOKIE_NAME = "unionx_cont_session"
COOKIE_MAX_AGE_HOURS = 8


def _cookie_secret() -> str:
    try:
        val = st.secrets.get("CONT_COOKIE_SECRET")
        if val:
            return str(val)
    except Exception:
        pass
    return os.environ.get("CONT_COOKIE_SECRET", "unionx-contabilidad-change-me")
...
def _get_cookie_manager():
    try:
        ...
        if "_cont_cookie_manager" not in st.session_state:
            st.session_state["_cont_cookie_manager"] = stx.CookieManager(
                key="cont_cookie_mgr"
            )
        return st.session_state["_cont_cookie_manager"]
    except Exception:
        return None
...
def _get_allowed_emails() -> list:
    try:
        val = st.secrets.get("CONT_ALLOWED_EMAILS") if hasattr(st, "secrets") else None
    except Exception:
        val = None
    if val:
...
def _login_form():
    ...
    with col2:
        ...
        st.markdown("---")
        with st.form("cont_login", clear_on_submit=False):
            email = st.text_input("Email Odoo")
            ...
            if submit:
                ...
                if not ok:
                    ...
                    return
                st.session_state["cont_user"] = {"email": email_l, "uid": uid}
...
def require_login_cont():
    if st.session_state.get("cont_user"):
        return
    cookie = _leer_cookie()
    if cookie:
        valid = _validar_cookie(cookie)
        if valid and valid["email"] in _get_allowed_emails():
            st.session_state["cont_user"] = valid
            return
    _login_form()
    st.stop()
...
def logout_cont():
    _eliminar_cookie()
    st.session_state.pop("cont_user", None)
    st.rerun()


def get_cont_user() -> dict | None:
    return st.session_state.get("cont_user")
